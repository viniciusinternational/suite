...
import { useCreateDeduction, useUpdateDeduction } from '@/hooks/use-deductions'
import axios from '@/lib/axios'
import type { Deduction, User, Department } from '@/types'
...
export function DeductionForm({ deduction, onSuccess }: Props) {
  ...
  const updateMutation = useUpdateDeduction(deduction?.id || '')
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      always: deduction?.always || false,
      amount: deduction?.amount || null,
      percent: deduction?.percent || null,
      global: deduction?.global || false,
      userIds: deduction?.users?.map((u) => u.id) || [],
      departmentIds: deduction?.departments?.map((d) => d.id) || [],
  ...
  const handleSubmit = async (data: FormValues) => {
    try {
      ...
      alert(error?.response?.data?.error || 'Failed to save deduction')
    }
  }
    ...
    <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          <Input id="title" {...form.register('title')} placeholder="e.g., Tax Deduction" />
