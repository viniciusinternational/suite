...
import { updateUserEmploymentSchema } from '../validation';
...
export async function PATCH(
  ...
  try {
    ...
    const parsedData = updateUserEmploymentSchema.parse(body);
    ...
    await logUserChange({
      ...
      existingUser,
      updatedUser,
      description: `Updated employment information for user "${updatedUser.fullName}"`,
    });
    ...
    return NextResponse.json(
      {
        ok: false,
        error: 'Failed to update user employment information',
      },
      { status: 500 }
    );
  }
